...
Rule dictionaries work by writing a keyword, then a rule value attached to it. For more information, look in [API Usage](https://github.com/Loepker-James/enforce-rules/blob/main/docs/api.md).
...
3. The helper may raise a ```ValueError```
...
Checks that the value itself is truthy when the rule is activated.

Failure means the value is falsy when the rule is activated.
...
## Failure Behavior
If any rule fails, the validator raises a ```ValueError```. The exact error message depends on the rule. Validation stops immediately when a rule fails, and no additional rules are checked.
